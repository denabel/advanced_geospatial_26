...
```{r}
CA_pop_stack <- terra::rast("./data/stacks/CA_pop_stack.tif")
...
ggplot()+
  ...
  labs(
    ...
    caption = "Source: WorldPop (www.worldpop.org)"
  ) +
  annotation_scale(
    location = "bl",
    width_hint = 0.3
...
```{r}
CA_pop_cube <- readRDS("./data/stacks/CA_pop_cube.rds")
...
## Exercise 6_1: A map with many facets {.center style="text-align: center;"}

[🖱 Click here to open the exercise](https://denabel.github.io/advanced_geospatial_26/exercises/6_1_Map_with_many_facets.html)

![](../img/cat_quality.png){width="15%" fig-align="center"}
...
Instead, an *Application Programming Interface* (API) is often used.
...
- Specialized `R` packages, such as the [`wiesbaden` package](https://cran.r-project.org/web/packages/wiesbaden/index.html), the [`tidycensus` package](https://cran.r-project.org/web/packages/tidycensus/index.html), or the [`z22` package](https://cran.r-project.org/package=z22) for German Census 2022 grid data (which we used throughout this course!)
...
```{r}
CA_nl_2020 <- terra::rast("./data/nightlights/VNP46A4_NearNadir_Composite_Snow_Free_qflag_t2020.tif")
...
LA_pop_2020 <- terra::rast("./data/worldpop/US-CA_ppp_2020_1km.tif") |> 
...
## Exercise 6_2: Illuminate your region {.center style="text-align: center;"}

[🖱 Click here to open the exercise](https://denabel.github.io/advanced_geospatial_26/exercises/6_2_Illuminate_your_region.html)
