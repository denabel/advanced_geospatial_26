...
```{r}
temp_base <- 
  terra::rast("./data/era5/era5_temperature1961-1990.grib")
```
...
First, we reshape the temperature anomalies into long format — one row per country and year, keeping the raw values in Kelvin. No standardization needed.
...
We deliberately move from individuals to country means — using the **same anomaly groups** as above. Compare the full concern distributions in the heatmap with these boxplots: the enormous individual variation collapses into a narrow band of country means.
...
- Country-level means compress the enormous individual variation into a narrow band
- The boxes barely differ across anomaly groups — the "signal" we might hope for is not there
- This makes the **aggregation problem** visible: country-level means compress enormous individual variation into a narrow band, and patterns at one level need not appear at the other — a reminder that cross-level inference (the "ecological fallacy") works in both directions
...
- This suggests that country-specific factors (media, politics, economic context) matter more than temperature alone

**Limitations**

- Only 4 time points — too few for robust trend analysis
- No causal claim possible: many confounding factors
- Finer spatial resolution (e.g., NUTS-2 regions) would strengthen the analysis
