...
```{r}
CA_pop_2017 <- terra::rast("./data/worldpop/US-CA_ppp_2017_1km.tif")
CA_pop_2018 <- terra::rast("./data/worldpop/US-CA_ppp_2018_1km.tif")
CA_pop_2019 <- terra::rast("./data/worldpop/US-CA_ppp_2019_1km.tif")
CA_pop_2020 <- terra::rast("./data/worldpop/US-CA_ppp_2020_1km.tif")
...
files <- list.files("./data/worldpop", pattern = "US-CA_ppp_20(17|18|19|20)_1km\\.tif$", full.names = TRUE)
...
## `terra::project()` documentation

<iframe
  src="../img/terra-project.html"
  width="100%"
  height="620"
...
| Method | Best for |
|---|---|
| `near` | categorical, IDs |
| `mode` | categorical |
| `bilinear` | continuous |
| `mean` | continuous, densities, proportions |
| `sum` | counts, totals |
| `median`, `q1`, `q3` | robust summaries |
| `min`, `max` | extremes |
| `cubic`, `cubicspline`, `lanczos` | smooth image-like continuous |
...
::: {.column width="50%"}
![](../img/cake_slice.gif){fig-align="center" width="50%"}
...
## Exercise 5_1: A raster stack with `terra` {.center style="text-align: center;"}

[🖱 Click here to open the exercise](https://denabel.github.io/advanced_geospatial_26/exercises/5_1_Raster_stack_terra.html)
...
```{r}
CA_pop_2017 <- read_stars("./data/worldpop/US-CA_ppp_2017_1km.tif")
CA_pop_2018 <- read_stars("./data/worldpop/US-CA_ppp_2018_1km.tif")
CA_pop_2019 <- read_stars("./data/worldpop/US-CA_ppp_2019_1km.tif")
CA_pop_2020 <- read_stars("./data/worldpop/US-CA_ppp_2020_1km.tif")
...
```{r}
files <- list.files("./data/worldpop",
...
In stars, `st_transform()` changes the CRS of the raster geometry without resampling the cell values. For regular gridded rasters, this produces a curvilinear grid in the target CRS. The cell values remain unchanged, so the operation is lossless with respect to the raster values. 
...
To obtain a regular grid is to create a workaround with  `st_warp()`. We first specify the target grid with same number of rows and columns and "warp" into that grid.
...
```{r}
saveRDS(CA_pop_cube, "./data/stacks/CA_pop_cube.rds")

write_stars(CA_pop_cube, 
            "./data/stacks/CA_pop_cube.nc",
            driver = "netCDF")
```
...
```{r}
# Transform terra SpatRaster into stars object
...
```{r}
# Transform stars object into terra SpatRaster
...
## Exercise 5_2: A raster cube with `stars` {.center style="text-align: center;"}

[🖱 Click here to open the exercise](https://denabel.github.io/advanced_geospatial_26/exercises/5_2_Raster_cube_stars.html)

![](../img/cat_measure.png){width="15%" fig-align="center"}
