...
-   Be comfortable with using **vector** AND **raster data** in `R`
...
- 🎥 **Camera on** -- if feasible; it makes the workshop feel much more interactive
- 🎙️ **Mute yourself** when not speaking to avoid background noise
- ✋ **Raise your hand** if you have a question 
- 💬 **Chat is your friend** -- feel free to post questions or comments anytime
- ☕ **Breaks are sacred** -- we'll stick to the schedule, use them to recharge
- 📂 **Work in `scripts/`** -- keep your own code there; but we may update other folders during the course
- 🆘 **Stuck?** -- share your screen, and we'll debug together
- 👥 **Breakout rooms** -- during exercises, we'll split into small groups so you can work together and help each other out

## Course materials

📥 [**Download**](https://github.com/denabel/advanced_geospatial_26/archive/refs/heads/main.zip)

🌐 [**Browse online**](https://denabel.github.io/advanced_geospatial_26/)
...
| Folder | Contents |
|---|---|
| `slides/` | Session slides (HTML, open in browser) |
| `exercises/` | Exercises with tips & solutions (HTML) |
| `data/` | Census grids, boundaries, survey data, … |
| `scripts/` | **Your workspace** — write your code here! |
| `R/` | Helper scripts (package installation, etc.) |

:::

::: {.callout-tip}
Open **`advanced_geospatial_26.Rproj`** in RStudio first — this sets all file paths correctly so the code works out of the box.
:::
...
- Climatological data from ERA5 reanalysis (C3S)
...
![](../img/flood_risk.png){fig-align="center" width="40%"}
...
## Refresher: Geodata basics

![](../img/f5_refresh.png){fig-align="center" width="20%"}
...
- **Geocentric datum**: Optimizes fit for entire Earth (like World Geodetic System 1984 - WGS84)
...
::::: columns
::: {.column width="50%"}
- PCS represents Earth's surface on a **flat plane** by mathematical transformations (projections)
...
-   [`z22`](https://cran.r-project.org/web/packages/z22/index.html)
:::
...
- German Census 2022 data are provided by the [Federal Statistical Office Germany, Wiesbaden 2024](https://www.zensus2022.de). We gathered the data using Jonas Lieth's [`z22` R package](https://cran.r-project.org/package=z22). The data live in `./data/z22/` in two formats:
  - **`.tif`** files: single-layer rasters (one variable, e.g., `population.tif`, `rent_avg.tif`)
  - **`.nc`** files: multi-layer rasters (categorical variables with multiple groups, e.g., `age_short.nc` contains age brackets)

  Layer names in `.nc` files can be cryptic — consult the [Census 2022 database](https://ergebnisse.zensus2022.de/datenbank/online/themes) for documentation, or use `z22::z22_categories()` if you have the package installed.
:::
...
## Exercise 1_1: Package Installation {.center style="text-align: center;"}

[🖱 Click here to open the exercise](https://denabel.github.io/advanced_geospatial_26/exercises/1_1_Package_Installation.html)

![](../img/cat_scientist.png){width="15%" fig-align="center"}
