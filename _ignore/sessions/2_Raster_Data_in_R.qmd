...
|---|---|
| `sf::read_sf()` / `sf::st_read()` | Load vector data (GeoJSON, Shapefile, …) |
| `sf::st_geometry()` | Extract geometry column (for plotting) |
| `sf::st_bbox()` | Get bounding box of a dataset |
| `sf::st_crs()` | Inspect coordinate reference system |
| `sf::st_transform(x, crs)` | Reproject to a different CRS |
| `sf::st_join(x, y)` | Spatial join — transfer attributes by location |
...
| Function | What it does |
|---|---|
| `terra::rast(file)` | Load a raster (`.tif`, `.nc`, …) or create from matrix |
| `terra::plot(raster)` | Quick visualization of raster layers |
| `terra::crs(raster, describe = TRUE)` | Inspect CRS of a raster |
| `terra::project(raster, crs)` | Reproject raster to a different CRS |
| `terra::global(raster, fun)` | Memory-efficient summary stats (mean, sum, …) |
| `terra::values(raster)` | Extract all cell values as a vector |
...
| Function | What it does |
|---|---|
| `stars::read_stars(file)` | Load raster as a `stars` object |
| `plot(stars_obj)` | Plot a `stars` raster layer |
