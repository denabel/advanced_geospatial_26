...
![](../img/fig_raster_processing.svg){width="85%" fig-align="center"}
...
## From survey points to a smooth density surface

The next slides will walk through a five-step pipeline: crop points → count on a coarse grid → reproject to a finer grid → mask to the study area → smooth with `focal()`.
...
## Steps so far for our 'real' point pattern analysis

![](../img/fig_point_to_raster_pipeline.svg){width="95%" fig-align="center"}

## Smoothing: 5×5 window

We can now also apply smoothing as with the population grid data.
...
Comparing the two columns, the difference is striking: the Census grid (left) captures fine-grained spatial variation because it covers the **entire population**, whereas our survey-based density (right) is driven by a sparse **sample** of point locations.

::: {.smaller-text}
::: {.fragment}
**What does this mean in practice?**

- Point-based density surfaces are useful for **exploratory visualization** and for detecting broad spatial clusters in your own data
- They are **not a substitute** for high-resolution administrative data (Census, register data) when precise local estimates matter
- The smoothing window size becomes a critical choice: too small → noisy artifacts from sparse data; too large → you blur away the very patterns you want to detect
:::

::: {.fragment}
::: {.callout-tip}
Always consider your **data source and sample size** before interpreting smoothed surfaces. A beautiful heat map can be misleading if the underlying point density is too low for the chosen resolution.
:::
:::
:::
