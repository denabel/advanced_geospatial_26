...
Load the four population raster layers for Texas 2017-2020 (`./data/worldpop/US-TX_ppp_2017_1km.tif`-`./data/worldpop/US-TX_ppp_2020_1km.tif`) with `stars::read_stars()` and integrate these into one raster cube. First, create a file list for all four raster layers. Second, prepare a vector which stores the four years. Finally, you can read in the file list along the list of years.
:::
...
files <- list.files("./data/worldpop", pattern = "US-TX_ppp_20(17|18|19|20)_1km\\.tif$", full.names = TRUE)
...
#| eval: false
# Export
saveRDS(TX_pop_cube, "./data/stacks/TX_pop_cube.rds")
```
